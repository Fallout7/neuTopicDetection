...
def main():
    ...
    if os.path.isfile('Prahy_neu.npy'):
        bestPrahACC, bestPrahF1, bestPrahHamm = np.load('Prahy_neu.npy')
    else:
        print('Nutné dodat prahy (získané z natrénované neu a validačních dat) pro určení témat z výstupu neuronové sítě.')
        exit()
    ...
    for id in nacteneSoubory:
        retezceTextyNahravky[id] = ' '.join(nacteneSoubory[id]["vse"]["slova"])
    ...
    for nahravka in lemmaTexty:
        ...
        x_test = vektor_TFIDF_nahravky
        predicted_probs = model1.predict(x_test)[0]
        topics, neco = {}, 0
        for i in range(len(predicted_probs)):
            hod = predicted_probs[i]
            if hod > bestPrahF1:
                prob = hod * 100.0
                topics[cisloToShl[i].strip()] = prob
                neco = 1
        if neco == 0:
            for i in range(len(predicted_probs)):
                hod = predicted_probs[i]
    ...
    file1.write(
        'Výsledky ' + str(int(len(results))) + ' vstupních souborů jsou uloženy do souboru : ' + str(args.output))
    file1.write('\n')
    for nahravka in results:
        file1.write('Nahrávce s id ' + str(nahravka) + ' jsou přiřazeny následující témata: ')
        file1.write('\n')
        file1.write(
            mezeraPred + '        Téma' + mezeraMezi[0:len(mezeraMezi) - len('Téma        ')] + 'Pravděpodobnost')
        file1.write('\n')
        for tema in results[nahravka]:
            file1.write(
                mezeraPred + str(tema) + mezeraMezi[0:len(mezeraMezi) - len(tema)] + str(results[nahravka][tema]))
            file1.write('\n')
        file1.write('\n')
    file1.close()
